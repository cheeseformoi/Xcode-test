...
struct ContentView: View {
    ...
    @State private var lines: [(CGPoint, CGPoint, CGPoint, Int)] = [] // Store completed lines, with (startPoint, adjustedEndPoint, actualEndPoint)
    ...
    var body: some View {
        VStack{
            Button(action: upload) {
                Group {
                    Spacer().frame(width: 0, height: 36, alignment: .topLeading)
                    HStack {
                        Text("HOME")
                            .bold()
                        Spacer()
            ...
            ZStack {
                    ...
                    .gesture(
                        DragGesture()
                            ...
                            .onEnded { _ in
                                ...
                                let errorNum = lines.count + 1
                                lines.append((startPoint, adjustedEndPoint, endPoint, errorNum)) // Store both adjusted and actual end points
                                isDragging = false
                            }
                    )
                ...
                ForEach(0..<lines.count, id: \.self) { index in
                    ...
                    drawLineWithCircle(start: line.0, adjustedEnd: line.1, actualEnd: line.2, lineNum: line.3)
                }
    ...
    func drawLineWithCircle(start: CGPoint, adjustedEnd: CGPoint, actualEnd: CGPoint, lineNum: Int) -> some View {
        return ZStack {
